...
export default {
	...
	execute: async (m) => {
		...
		if (isFetchAll) {
			await m.reply("⟳ Lagi ambil full list Anichin Completed. Tunggu sebentar, lagi diproses...");
		} else {
			await m.reply(`⟳ Lagi proses data Anichin Completed halaman ${targetPage}...`);
		}
		...
		try {
			...
			while (true) {
				const url = currentPage === 1 ? "/completed/" : `/completed/page/${currentPage}/`;
...
text += `${!isFetchAll ? `📄 Halaman ${targetPage}\n` : `📦 Semua Data\n`}\n`;
			...
			console.error("[ANICHIN ERROR]:", error.message);
